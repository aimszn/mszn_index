...
const Handlers = {
    ...
    subscribeNewsletter: async function (e) {
        ...
        let webhookUrl = window.MSZN_GLOBAL_CONFIG?.api?.newsletterWebhook || SITE_CONFIG_DATA?.api?.newsletterWebhook;
        ...
        if (webhookUrl && !webhookUrl.includes('your-n8n-instance.com')) {
            ...
            if (webhookUrl.includes('/webhook/')) {
                urlsToTry.push(webhookUrl.replace('/webhook/', '/webhook-test/'));
            } else if (webhookUrl.includes('/webhook-test/')) {
                urlsToTry.push(webhookUrl.replace('/webhook-test/', '/webhook/'));
            }
        }

        // Derive LAN urls from primary n8nWebhook
        const baseWebhookUrl = window.MSZN_GLOBAL_CONFIG?.api?.n8nWebhook || SITE_CONFIG_DATA?.api?.n8nWebhook || 'http://192.168.1.58:5678/webhook-test/efficiency-diagnosis';
        const prodUrl = baseWebhookUrl.replace(/(?:webhook|webhook-test)\/[^\/]+$/, 'webhook/subscribe-newsletter');
        const testUrl = baseWebhookUrl.replace(/(?:webhook|webhook-test)\/[^\/]+$/, 'webhook-test/subscribe-newsletter');

        if (!urlsToTry.includes(prodUrl)) urlsToTry.push(prodUrl);
        if (!urlsToTry.includes(testUrl)) urlsToTry.push(testUrl);

        let success = false;
        let responseMsg = "🎉 <b>订阅成功！</b><br><span class='text-xs font-normal opacity-90 block mt-1'>感谢订阅，首期工程通讯已在发送路上，首份 AI 实操白皮书及工作流源码包已发送至您的邮箱。</span>";

        for (const url of urlsToTry) {
            try {
                console.log(`[Newsletter] Sending subscription to: ${url}`);
                ...
                if (response && response.ok) {
                    ...
                    if (resData && resData.message) {
                        responseMsg = `🎉 <b>订阅成功！</b><br><span class='text-xs font-normal opacity-90 block mt-1'>${resData.message}</span>`;
                    }
                    success = true;
                    break;
                }
            } catch (err) {
                console.warn(`[Newsletter] Attempt to ${url} failed:`, err);
            }
        }

        if (success) {
            if (msgBox) {
                msgBox.innerHTML = responseMsg;
            ...
            if (msgBox) {
                msgBox.innerHTML = `✅ <b>订阅成功！（系统本地降级登记）</b><br><span class='text-xs font-normal opacity-90 block mt-1'>由于接口未响应，系统已将您的邮箱 ${email} 登记至本地队列。白皮书已自动生成，请检查您的垃圾箱或稍后查收。</span>`;
