// ============================================
// 麻升智能工作室 - 沉浸式工作台
// 宋词点唱机 (Song Poetry Jukebox) Component
// ============================================
...
Object.assign(window.WorkspaceUI, {
                            ...
                            <div class="flex items-center gap-4 bg-slate-900/60 border border-slate-800 p-4 rounded-2xl relative" id="step-node-1">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex justify-between items-center mb-1">
                                        <span class="text-sm font-bold text-white">步骤一：词作意境文学 analysis</span>
    ...
    updateSingerProfile: function() {
        ...
        const profiles = {
            'auto': {
                ...
                desc: "大模型将自动对您的宋词进行情感温度提取，并选取最契合 the 数字歌姬进行声音特征灌装。"
            },
            '柳如是': {
                name: "柳如是 · 儒服剑胆",
            ...
            '董小宛': {
                ...
                desc: "淡雅清高，琴艺精湛。声线清甜细腻，伴奏以极简古琴、古筝为主，空灵慢旋。"
            },
            '李香君': {
                name: "李香君 · 血溅桃花",
    ...
    togglePoetryAudio: function() {
        ...
        if (WorkspaceUI.audioPlaying) {
            ...
            const playGuzhengPluck = (freq, delay = 0) => {
                ...
                gainNode.gain.linearRampToValueAtTime(0.18, ctx.currentTime + delay + 0.015);
                gainNode.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + delay + 2.5);
    ...
    copyPoetryResult: function() {
        ...
        if (text) {
            WorkspaceUI.copyTextToClipboard(text);
        } else {
            const content = document.getElementById('poetry-result-content');
            if (content) {
                WorkspaceUI.copyTextToClipboard(content.innerText);
            }
        }
    },
    ...
    renderResultDashboard: function(title, author, singer, data) {
        ...
        resultContent.innerHTML = `
            <div class="grid grid-cols-1 lg:grid-cols-5 gap-6 h-full max-w-6xl mx-auto py-1 text-left">
                ...
                <!-- Right Panel: Tabs for Lyrics, Card, Prompts -->
...
### ✦ ━━ 企划手记 ━━ ✦
在浩瀚的历史长河中，这首《${title}》所蕴含的意境，由${singerName}来演绎，实乃天作之合。她那独特的声线质感与极具张力的演唱，将词中的凄美与风骨表现得淋漓尽致，堪称数字人文的史诗级碰撞。
